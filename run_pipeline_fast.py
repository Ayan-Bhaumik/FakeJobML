#!/usr/bin/env python3
"""
Fast pipeline for Fake Job Posting Detection
Optimized for speed - skips slow models, uses fewer CV folds
"""
...
print("=" * 60)
print("FAKE JOB POSTING DETECTION - FAST PIPELINE")
print("=" * 60)
...
# TF-IDF only for Naive Bayes
X_train_tfidf = X_train[:, :3000]
X_test_tfidf = X_test[:, :3000]
...
# ============================================================
# 7. DEFINE MODELS (FAST VERSION)
# ============================================================
print("\n[7/10] Defining models...")

# Use LinearSVC with calibration instead of SVC for speed
...
# ============================================================
# 8. TRAIN AND EVALUATE (NO CV FOR SPEED)
# ============================================================
print("\n[8/10] Training and evaluating models...")

def train_and_evaluate(model, X_train, y_train, X_test, y_test, model_name, use_tfidf_only=False):
    print(f"\n{'='*50}")
...
if hasattr(best_model, 'feature_importances_'):
    ...
    # For CalibratedClassifierCV, get from base estimator
    base_est = best_model.calibrated_classifiers_[0].estimator
    if hasattr(base_est, 'coef_'):
        importances = np.abs(base_est.coef_[0])
    else:
        importances = None
else:
    importances = None
